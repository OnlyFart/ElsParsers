...
namespace StudentLibrary.Parser.Logic {
    public class Parser : ParserBase {
        ...
        public async Task<BookInfo> GetBook(HttpClient client,
            Uri uri) {
            ...
            var isbn = uri.Segments.Last().Split(".").First().Split(new[] { "ISBN" }, StringSplitOptions.None).Last();
            var book = new BookInfo(isbn, ElsName) {
                ISBN = isbn,
        ...
        private static IEnumerable<Uri> Filter(IEnumerable<Uri> uris,
            ...
            foreach (var uri in uris) {
                var idStr = uri.Segments.Last().Split(".").First().Split(new[] { "ISBN" }, StringSplitOptions.None).Last();
